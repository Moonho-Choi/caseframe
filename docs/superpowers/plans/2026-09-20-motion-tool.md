...
- 지원 브라우저: 크롬·엣지(WebGPU+WebCodecs). WebGPU가 없으면 단순 겹치기로 자동 전환.
  (2026-09-21 개정: WebCodecs가 없으면 WebM으로 전환하지 않고 지원하지 않는다고 안내한다 —
  사파리 16.4+·파이어폭스 130+도 VideoEncoder가 있어 그 예비 경로는 실제로 골라진 적이 없고,
  브라우저에서 한 번도 돌려 본 적 없는 코드였다. 설계 문서 §5.6·§6 참조.)
...
- 방향 검사 규칙(§5.2): 뒤집은 쪽 점수 > 원본 × 1.3 이고 ≥ 15 → 반전. 두 쪽 모두 < 8 → "다른 방향" 경고.
  (2026-09-21 개정: 사진 사이의 상대 방향을 사슬로 이어 붙이는 방식으로 바꾸고 문턱을 `REL_MIN = 8` 하나로
  통일했다. 성긴 6장 묶음에서 15점 문턱이 명백한 관계까지 놓쳤기 때문이다. 설계 문서 §5.2 참조.)
...
test('alignedSize is even and 90% of frame', () => {
  assert.deepEqual(alignedSize(1280, 854), { cw: 1152, ch: 768 });   // 2026-09-21 개정: 16의 배수로 내림
});
...
- 내려받은 MP4를 `ffprobe`로 확인: h264, 1152x768(2026-09-21 개정: 잘라낸 크기를 16의 배수로 내림), fps≈53, 길이 ≈ 24×1.2+1 = 29.8초.
